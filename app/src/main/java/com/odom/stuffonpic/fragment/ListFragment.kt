...
import com.odom.stuffonpic.adapter.ListviewAdapter
...
class ListFragment : Fragment() {
    ...
    var itemsEmpty = ArrayList<String>()
    ...
    private fun setLayoutFragment() {
        ...
        // 리스트있으면 보여주기
        if(getStringArrayPref("listData") != null){
            items = getStringArrayPref("listData")!!

            if(items.size > 0) {
                for(i in 0 until items.size){
                    listviewAdapter.addItem(items[i])
                }
    ...
    private fun addList(){
        ...
        val item1 = et_workoutName.text.toString()
        ...
        // 운동종목 저장
        listviewAdapter.addItem(item1)
        listviewAdapter.notifyDataSetChanged()
    }

    private fun dialogUpdateDelete(list: ArrayList<String>, position: Int) {
        ...
        workoutNameText.setText(list[position])

        val mAlertDialog = mBuilder.show()
        // 수정
        bt1.setOnClickListener {
            list[position] = workoutNameText.text.toString()
